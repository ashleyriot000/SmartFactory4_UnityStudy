...
public class Actor : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        //transform의 position와 rotation을 변경하면 그 위치,회전 상태로 변경할 수 있음.
        ...
        //transform의 rotation에 사원수값을 대입하면 회전함.
        //360도로 정의된 Vector3값을 Quaternion.Euler()함수를 이용하면 사원수로 변환해줌.
    ...
    void Update()
    {
        #region 이동, 회전 관련된 다른 함수들
        //지정된 방향으로 원하는 거리만큼 이동하는 함수
        //transform.Translate(Vector3.forward * moveSpeed * Time.deltaTime);

        //현재 회전값에서 목적 회전값까지 최대 회전각도만큼 회전하는 함수
        //transform.rotation = 
        //    Quaternion.RotateTowards(transform.rotation, Quaternion.Euler(new Vector3(30, 30, 30)), 90f * Time.deltaTime);
        #endregion

        //현재 위치에서 목적지까지 최대거리만큼 이동하는 함수
        transform.position =
            Vector3.MoveTowards(transform.position, moveTransforms[_currentIndex].position, moveSpeed * Time.deltaTime);
